(function () {
  ...
  function selectCard(card) { state.review.selected = card; const story = currentStory(); if (story) rememberActiveReview({story_token: story.story_token, draft_token: state.review.token, card_id: card.card_id}); setReviewActions(true, card.kind === 'line' || card.kind === 'unknown'); $('#rvSelectionLabel').textContent = '已选 #' + card.line_no + ' · ' + (card.kind === 'line' ? '台词' : card.kind === 'dir' ? '指令' : card.kind === 'unknown' ? '未识别行' : '卡片'); applyCardSelection(); applyTimelineSelection(); const player = playerInstance(); if (player && typeof player.jumpToCard === 'function') player.jumpToCard(card.card_id); }
  ...
  function editCard(trigger) { const card = state.review.selected; if (!card) return; const current = card.current || {}; $('#editWho').value = current.who || ''; $('#editText').value = card.kind === 'line' ? current.text || '' : card.kind === 'unknown' ? String(card.raw || '').replace(/\r?\n$/, '') : current.arg || ''; ['Face', 'Emo', 'Act', 'Fx'].forEach(function (key) { $('#edit' + key).value = current[key.toLowerCase()] || ''; }); $('#editTitle').textContent = '编辑卡片 #' + card.line_no; openModal('#mEdit', trigger); }
  async function saveEdit() { const card = state.review.selected; if (!card) return; const patch = (card.kind === 'line' || card.kind === 'unknown') ? {who: $('#editWho').value.trim(), text: $('#editText').value, face: $('#editFace').value.trim(), emo: $('#editEmo').value.trim(), act: $('#editAct').value.trim(), fx: $('#editFx').value.trim()} : {arg: $('#editText').value.trim()}; try { await reviewPost('/api/cards/update', {card_id: card.card_id, patch: patch}); closeModal('#mEdit'); await loadReview(); } catch (error) { status(error.message); } }
  ...
  async function bindCast() { const card = state.review.selected; if (!card || (card.kind !== 'line' && card.kind !== 'unknown')) return; if (card.kind === 'unknown') { const who = window.prompt('输入该句的说话者名称；留空设为旁白', ''); if (who === null) return; try { await reviewPost('/api/cards/update', {card_id: card.card_id, patch: {who: who, text: String(card.raw || '').replace(/\r?\n$/, '')}}); await loadReview(); } catch (error) { status(error.message); } return; } const ident = window.prompt('输入 AA 角色 ident；留空设为旁白', ''); if (ident === null) return; try { await reviewPost('/api/draft/cast/update', {speaker: card.current.who, mapping: ident ? {id: ident, name: card.current.who, portrait: true} : {narrator: true}}); await loadReview(); } catch (error) { status(error.message); } }
