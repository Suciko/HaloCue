...
describe("scene evaluation seam", () => {
  ...
  it("binds the selected descriptor to a deterministic timeline", () => {
    ...
    expect(evaluation.schema_version).toBe("scene-evaluation/1.5");
  ...
  it("reports a motion placed outside the target character's occupied range", () => {
    const project = structuredClone(demoProject);
    ...
    expect(evaluation.performance.source_map.some((entry) => (
