import type {
  ...
  RenderTimeline,
  SceneDescriptor,
  ScenePerformanceOperation,
  ScenePerformancePlan,
  ScenePerformanceSample,
} from "./types";

export const PERFORMANCE_SCHEMA_VERSION = "scene-performance/1.4" as const;
...
export const SHAKE_MAX_X_PX = 14;
export const SHAKE_MAX_Y_PX = 8;
export const CHARACTER_ENTER_OFFSET_Y_PX = 24;
...
function validateInputs(descriptor: SceneDescriptor, timeline: RenderTimeline): void {
...
export function buildScenePerformance(
  descriptor: SceneDescriptor,
  timeline: RenderTimeline,
): ScenePerformancePlan {
  validateInputs(descriptor, timeline);
  const operations: ScenePerformanceOperation[] = [];
  const slotCharacters = new Map<number, string>();
  ...
  const addCharacterTween = (
    ...
    slot: number,
    suffix: string,
    channel: "presentation.opacity" | "layout.offset-y" | "presentation.scale",
...
function requireFrame(plan: ScenePerformancePlan, frame: number): number {
...
export function sampleScenePerformance(
  ...
  for (const operation of active) {
    ...
    const localFrame = resolvedFrame - operation.start_frame;
