...
export const PERFORMANCE_SCHEMA_VERSION = "scene-performance/1.0" as const;
...
export function buildScenePerformance(
  ...
  const operations: StageShakeOperation[] = [];
  for (const item of timeline.events) {
    if (item.kind !== "halocue.ba:screen-shake") continue;
