...
- 确认旁白、无立绘角色和演员映射。制作工作台可以提前创建老师的空立绘身份。
...
在制作工作台中，选中原文里代表老师的说话人，点击“老师身份”，再保存预设：
`sensei / 沙勒`、`sensei / 夏莱`、`老师 / 沙勒`、`老师 / 夏莱`，也可自定义名称和组织。
组织可以留空；创建身份不消耗模型调用，也不要求导入一张空白图片。

同一制作任务中的老师保持同一个角色 ID。多种原文称呼可明确绑定到该身份，
之后修改名称或组织会同步这些绑定，不会改写剧本原文，也不会把其他无立绘角色当成老师。
编译时老师使用 AA 槽 0，不占五个人物站位；安装仍需要单独确认。

本次支持槽 0 空立绘老师流程，也支持单回答 Sel 呈现；多分支选择仍未包含。
接口、错误码和兼容边界见 [制作服务说明](services/halocue/production/README.md#teacher-identity-presets)。
