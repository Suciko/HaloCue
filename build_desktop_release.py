"""Build and validate the HaloCue 0.9.4.5 Windows desktop release."""
...
PRIVATE_RELEASE_BASENAME = f"HaloCue-{VERSION}-private-webgl-lite-windows-x64"
...
def build_release(
    ...
    *,
    ...
    (release_dir / "版本.txt").write_text(
        f"HaloCue {VERSION}\nWindows 10/11 x64\n"
        + ("Private build with local Spine WebGL runtime\n" if private_spine_source else ""),
        encoding="utf-8",
    )

    if private_spine_source:
        # WebGL runtimes are packaged with the page assets.  Do not copy the
        # full Spine editor/CLI into a portable build: it is unnecessary for
        # local WebGL rendering and was the largest part of the private ZIP.
        (release_dir / "SPINE-NOTICE.txt").write_text(
            "This private build renders Spine assets with the bundled local "
            "Spine WebGL runtimes.  The Spine runtimes remain subject to "
            "Esoteric Software LLC's runtime license.\n",
            encoding="utf-8",
        )
