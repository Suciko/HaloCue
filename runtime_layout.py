...
class RuntimeLayout:
    ...
    def thumbs_root(self) -> Path:
        return self.user_data_root / ".thumbs"

    @property
    def legacy_config_path(self) -> Path:
        return self.resource_root / "aa_config.json"
...
def resolve_runtime_layout(
    *,
    ...
    packaged_seed = resources / "aa_assets.db"
    if not packaged_seed.is_file():
        packaged_seed = resources / "data" / "halocue_labels.db"
    ...
    return RuntimeLayout(
        ...
        database_seed_path=packaged_seed.resolve(),
