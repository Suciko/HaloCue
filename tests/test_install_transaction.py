...
@pytest.fixture
def temp_environment(tmp_path, monkeypatch, seed_draft_resources):
    ...
    create_draft = store.create_draft

    def create_draft_with_resources(*args, **kwargs):
        result = create_draft(*args, **kwargs)
        token = kwargs.get("token") or args[0]
        seed_draft_resources(store, token)
        return result

    monkeypatch.setattr(store, "create_draft", create_draft_with_resources)
