...
def test_main_with_mock_provider_writes_a_portrait_annotation(
    tmp_path, monkeypatch, empty_llm_config_path
):
    ...
    monkeypatch.setattr(sys, "argv", [
        ...
        "--index", str(index), "--llm", str(empty_llm_config_path),
        "--provider", "mock",
    ])
    monkeypatch.setattr(
        annotate,
...
def test_agent_mode_accepts_mock_provider_source_identity_response(
...
def test_agent_reuses_one_source_prefixed_static_prompt_across_chunks(
    tmp_path, empty_llm_config_path
):
    script = tmp_path / "scene.txt"
    ...
    annotate.annotate_script({
        ...
        "cast": str(cast), "index": str(index),
        "llm": str(empty_llm_config_path), "agent_enabled": True,
        "checkpoint_dir": str(tmp_path / "checkpoints"),
...
def test_confirmed_usage_chain_is_sent_as_annotation_context(
    tmp_path, empty_llm_config_path
):
    script = tmp_path / "scene.txt"
    script.write_text("Kai: hello\n", encoding="utf-8")
    ...
    annotate.annotate_script({
        "script": str(script), "out": str(tmp_path / "annotated.txt"),
        "cast": str(cast), "index": str(index),
        "llm": str(empty_llm_config_path), "usage_chain": plan,
    }, provider_instance=Provider())
