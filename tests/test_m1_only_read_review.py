...
def test_m1_synthetic_annotated_sample_import_and_review(
    tmp_path, annotated_script_path
):
    # 1. 注册 file_token
    ft_token = register_file_token(str(annotated_script_path))
    real_path = resolve_file_token(ft_token)
    assert real_path == str(annotated_script_path.resolve())
